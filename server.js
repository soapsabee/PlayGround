...
app.get("/", (req, res) => {
  axios
    .all([
      axios.get(
        `http://localhost:8080/api/find_AppByInstall/?installs=5,000,000`
      ),
...
app.get("/appby_name", (req, res) => {
  ...
  axios
    .get(`http://localhost:8080/api/find_AppByName/?name=${name}`)
...
app.get("/show_apps/:category", (req, res) => {
  ...
  axios
    .get(`http://localhost:8080/api/find_AppByCategory/?category=${category}`)
    .then(function(response) {
      let product = [];
      response.data.map(posts => {
        product.push(posts);
...
app.get("/appnews", (req, res) => {
  axios
    .get(`http://localhost:8080/api/find_AppByNews`)
    .then(function(response) {
      let product = [];
      response.data.map(posts => {
        product.push(posts);
...
app.get("/appMostlyInst", (req, res) => {
  axios
    .get(`http://localhost:8080/api/find_AppByInstall/?installs=5,000,000`)
    .then(function(response) {
      let product = [];
      response.data.map(posts => {
        product.push(posts);
...
app.get("/appby_type/:type", (req, res) => {
  ...
  axios
    .get(`http://localhost:8080/api/find_AppByType/?type=${type}`)
    .then(function(response) {
      let product = [];
      response.data.map(posts => {
        product.push(posts);
...
app.get("/appby_rate", (req, res) => {
  ...
  axios
    .get(`http://localhost:8080/api/find_AppByRate/?rating=${rate}`)
    .then(function(response) {
      let product = [];
      response.data.map(posts => {
        product.push(posts);
...
app.get("/review_app/:app/:category", (req, res) => {
  ...
  axios
    .all([
      ...
      axios.get(
        `http://localhost:8080/api/find_AppByCategory/?category=${category}`
      )
    ])
    .then(
      axios.spread((response_1, response_2, response_3) => {
        let product = [];
        let product2 = [];
        let product3 = [];
...
app.get("/api/find_AppByPrice/:price1/:price2", (req, res) => {
  ...
  axios
    .get(
      `http://localhost:8080/api/find_AppByPrice/?price1=${price1}&price2=${price2}`
    )
    .then(function(response) {
      let product = [];
      response.data.map(posts => {
